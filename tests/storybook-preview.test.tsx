...
const mockStory = [
  {
    ...
    imageData: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=', // Raw base64
    mimeType: 'image/png',
  },
  {
    chapter: 2,
...
describe('StorybookPreview', () => {
  ...
  it('should render chapter images with correct src and alt attributes', () => {
    ...
    expect(images[0]).toHaveAttribute('src', `data:${mockStory[0].mimeType};base64,${mockStory[0].imageData}`);
    expect(images[0]).toHaveAttribute('alt', `Illustration for Chapter ${mockStory[0].chapter}`);

    expect(images[1]).toHaveAttribute('src', `data:${mockStory[1].mimeType};base64,${mockStory[1].imageData}`);
